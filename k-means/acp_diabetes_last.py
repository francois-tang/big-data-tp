...
# Méthode du coude
elbow_method(Xpca, 50)

# Méthode silhoutte
silhouette_method(Xpca, 50)

# Application de K-means
kmeans = KMeans(n_clusters=3)  # Spécifier le nombre de clusters souhaité
kmeans.fit(Xpca)
kmeans_labels = kmeans.labels_

# Visualisation des clusters obtenus
plt.scatter(Xpca.CP_1, Xpca.CP_2, c=kmeans_labels) #, c=kmeans_labels , c=data.Insulin
plt.xlabel('PC1')
plt.ylabel('PC2')
...
data_filtered = data[data.kmeans_labels == 2]

ax = sns.heatmap(data_filtered.corr(), annot=True, fmt=".2f", linewidths=.5, vmin=-1, vmax=1)
